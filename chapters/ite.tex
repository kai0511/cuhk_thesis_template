...
\label{sec:ite_res}
  ...
  \subsection{Applications to Real Data}
    We has witnessed the global outbreak of Coronavirus disease (COVID-19), and it has been seriously affecting our daily life. Realtime data source shows that COVID-19 has infected over 15 million  people and caused more 600 thousands deaths throughout the world \cite{biobank2014uk}. Most infected people experience mild to moderate symptoms and recover without special treatment, but a subgroup of patients has severe syndromes, including acute respiratory distress syndrome (ARDS) \cite{ellinghaus2020genomewide}. We are interested in the wide difference in severity of syndrome among the infected people, that is, why most infected people have mild or even no symptoms and some experience serious symptoms like ARDS. Here we aim to identify clinical risk factors that lead to the heterogeneity of severity and to look for clinical or genetic explanation for causal relationship.
    ...
    We applied our ITE framework to GWAS data of patients with COVID-19. The data was extracted from UK Biobank, a large long-term biobank study in the United Kingdom, which aims to investigate genetic heritability and environmental exposure to the development of disease \cite{biobank2014uk}. We considered clinical factors studies in \cite{atkins2020preexisting} and genetic expression information imputed by PrediXcan \cite{gamazon2015gene}, and only included COVID-19 test positive subjects. The imputation of gene expression by PrediXcan is tissue specific, so in the study we examined the imputed gene expression of blood and lung.
    
    The severity for patients was defined as whether the patient was inpatient. If a patient was received inpatient treatment, the severity is 1; otherwise, the severity is 0. Since there were missing values for some clinical variables in the data, we imputed the missing values using missForest \cite{stekhoven2012missforest} with default setting, which iteratively employs random forests to impute the missing values.

    In practice, we considered one clinical variable as treatment, severity as outcome variable, and all other clinical and genetic factors as covariates for each run. If there were at least two out of three p-values $< 0.05$ from the split-half correlation (SHC) for specific clinical variable, then permutation tests would be carried out to further validate the fitting of causal forests. Practically, genetic factors for lung and blood were modeled separately with clinical variables. Results for blood and lung are summarized in Table \ref{tab:ite_blood_result} and \ref{tab:ite_lung_result} respectively. 
    ...
    \begin{table}[htbp]
      ...
      \begin{threeparttable}
        ...
        Diabetes diagnosed  & 3.736E-54 & 1.575E-17 & 2.063E-17 & \textbf{0.000} & \textbf{0.000} \\
    ...
    Clinically, HbA1c is a blood test that is used to diagnose and monitor people with diabetes. Our results indicate that diabetes diagnosed and HbA1c have a strong causal effect on severity of COVID-19 infection, with p-values of SHC  $<$ 5E-17 on lung data and $<$ 1E-11 on blood data and of permutation test $<$ 0.001 on lung data and $<$ 0.07 on blood data. Previous studies also confirm our results \cite{mehra2020cardiovascular,fang2020patients,zhu2020association}. A study shows that patients with type 2 diabetes usually are treated with ACE inhibitors, resulting in an upregulation of ACE2, but the upregulation of ACE2 facilitates infection with COVID-19 \cite{fang2020patients}. Our results also reveal cholesterol, HDL cholesterol and LDL direct are possible risk factors, causing severe infection among people with COVID-19. These clinical variables are associated with diagnose of cardiometabolic diseases, and studies have demonstrated that cardiometabolic comorbidities are related to a more severe course of COVID-19 \cite{stefan2020obesity,guo2020cardiovascular,shi2020association}.
    
    Interestingly, our study reveals that alcohol drinkers are at high risk of developing serious COVID-19 infection. Meanwhile, previous studies concluded that alcohol consumption (also social-moderate) is associated with the amount of ACE2 present in the body and particularly in lung \cite{okuno1986mild,testino2020patients}. This claim supports our findings, but it may be subjected to confounding variables like sharing alcoholic drinks \cite{mungmungpuntipantip2020sharing} and social gathering \cite{andersen2020early}.

    Our study also shows ethnic background and age might be potential risk factors which increase the severity of infection. This also is proved by another study , demonstrating that black ethnicity are at higher risk and South Asians and other ethnicities have intermediate risks compared with the white, and that people aged 80+ years are more likely to have severe infection \cite{atkins2020preexisting}.

\section{Conclusion}
\label{sec:ite_conclusion}
    

  In the chapter, we proposed a computational framework to estimate individualized treatment effects (ITE) of risk factors on patient outcome. In order to assess model fitting, we propose three different criteria to investigate the model fitting. In order to increase the generality of ITE framework, we proposed an approach based on weighted "mean imputation" to incorporate survival times as outcomes. We carried out simulations to validate our framework on survival outcomes, since if our approach could work in survival data, then it's more readily to accommodate other kinds of data. Cross all simulation scenarios, our methods showed a strong power in either constant or heterogeneous treatment effects. In real data analysis, we applied our approach to GWAS data of patients with COVID-19, in which the outcome variable is severity of COVID-19 infection, and our results are supported by previous studies.

  Our approach may have following limitations. In practice, we found that split-half correlation method has better power than permutation based methods. We conjecture that this is due to the characteristics of GWAS data, in which each covariate has very small effect and lots of covariates contribute to the outcome. We are investigating this issue, and our preliminary study confirmed this issue. In simulation scenario 8, there are much more covariates contributing to the outcome, then correlation based methods showed better power. In future, 
